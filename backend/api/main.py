...
# Train classifier once on startup
...
def analyze(req: TICRequest):
    ...
    logger.info(f"Analyzing TIC {req.tic_id}")
    ...
    try:
        ...
        if lc is None:
            raise HTTPException(
                status_code=404,
                detail=f"No TESS data found for TIC {req.tic_id}"
            )
        time, flux, flux_err = extract_arrays(lc)
        logger.info(f"Downloaded {len(time)} data points")
    ...
    try:
        ...
        logger.info(f"Cleaned. Scatter: {cleaned['scatter_ppm']:.1f} ppm")
    except Exception as e:
    ...
    try:
        ...
        logger.info(f"BLS: Period={bls_result['period']:.4f}d, SNR={bls_result['snr']:.1f}")
    except Exception as e:
    ...
    try:
        ...
        logger.info(f"Classification: {classification['label']} ({classification['confidence']:.1f}%)")
    except Exception as e:
    ...
    try:
        ...
        duration_validation = validate_duration(
            depth_result["duration_hrs"],
            refined_period
        )
        radius_ratio = np.sqrt(bls_result["depth"]) if bls_result["depth"] > 0 else 0
    except Exception as e:
        logger.warning(f"Fitting failed: {e} — using BLS estimates")
        ...
        duration_validation = {"flag": "UNKNOWN", "interpretation": "Fitting unavailable"}
        radius_ratio = np.sqrt(bls_result["depth"]) if bls_result["depth"] > 0 else 0
    ...
    try:
        ...
        figure_url = f"/outputs/orbitai_TIC{req.tic_id}.png"
        logger.info(f"Figure saved: {figure_url}")
    except Exception as e:
    ...
    result = {
        ...
        "duration_validation": {
            "flag":           duration_validation["flag"],
            "interpretation": duration_validation["interpretation"]
        },
...
@app.get("/results")
def list_results():
    """List all processed stars — used for batch results table."""
    results = []
...
def delete_result(tic_id: int):
    """Delete cached result for a star."""
    path = f"outputs/result_TIC{tic_id}.json"
    fig_path = f"outputs/orbitai_TIC{tic_id}.png"

    if not os.path.exists(path):
        raise HTTPException(status_code=404, detail=f"No result found for TIC {tic_id}")
